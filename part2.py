...
def kMeansClusture(k,data,datainclusters):
	...
	curmeans=numpy.zeros((CURRENTNOOFCLUSTERS,VECTORDIMENSION),dtype=numpy.float)
	initialMeanIndex=[]
	...
	while(True):
		...
		newmeans=numpy.zeros((CURRENTNOOFCLUSTERS,VECTORDIMENSION),dtype=numpy.float)
...
def NormalDist(data,mean,covariance):
	...
	N=math.exp(-0.5*temp[0][0])/math.sqrt(2*3.14)*math.sqrt(numpy.linalg.det(covariance))
	return N
...
mean=numpy.zeros((noofdir-1,CURRENTNOOFCLUSTERS,VECTORDIMENSION),dtype=numpy.float)
covariance=numpy.zeros((noofdir,CURRENTNOOFCLUSTERS,VECTORDIMENSION,VECTORDIMENSION),dtype=numpy.float)
...
#				print "\bMAXIMIZING MEAN CLASS :",(clsi+1),"PROGRESS :",round(100*float(datai+1)/float(len(data[clsi])),2),"%",
				mean[clsi][clusi]=funcs.addM(mean[clsi][clusi],funcs.mulByConstM(data[clsi][datai],gammak[clsi][datai][clusi][0]/NK[clsi][clusi]))
...
#				print "\bMAXIMIZING COVARIANCE CLASS :",(clsi+1),"PROGRESS :",round(100*float(datai+1)/float(len(data[clsi])),2),"%",
				covariance[clsi][clusi]=funcs.addM(funcs.mulByConstM(numpy.dot(funcs.subM(data[clsi][datai],mean[clsi][clusi]),funcs.transpose(funcs.subM(data[clsi][datai],mean[clsi][clusi]))),gammak[clsi][datai][clusi][0]/NK[clsi][clusi]),covariance[clsi][clusi])
...
#as data is not going to converge -- REAL IMAGE DATA
	# if(emCheckConvergence(1,100,interationNo)):
	# 	break
